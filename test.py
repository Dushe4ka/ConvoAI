...
import time
import markdown  # библиотека для преобразования Markdown в HTML
...
@app.post("/new_chat")
async def create_new_chat(request: NewChatRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        new_chat = Chat(session_id=new_session_id, chat_name=request.chat_name, collection_name=collection_name)
        db.add(new_chat)
        db.commit()
...
async def get_chats(db: Session = Depends(get_db)):
    ...
    return [{"session_id": chat.session_id, "chat_name": chat.chat_name, "collection_name": chat.collection_name} for chat in chats]

# --- Удаление чата и коллекции ---
@app.delete("/chat/{session_id}")
async def delete_chat(session_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        qdrant_client.delete_collection(collection_name=chat.collection_name)
        logger.info(f"Коллекция {chat.collection_name} удалена")

        # Удаляем чат из БД
        db.delete(chat)
        db.commit()

        return PlainTextResponse("Чат и коллекция удалены.")
    except Exception as e:
        logger.error(f"Ошибка удаления чата: {e}")
        raise HTTPException(status_code=500, detail="Ошибка удаления чата")

# --- Переключение между чатами ---
@app.post("/switch_chat/{session_id}")
async def switch_chat(session_id: str, db: Session = Depends(get_db)):
    chat = db.query(Chat).filter(Chat.session_id == session_id).first()
    if not chat:
        raise HTTPException(status_code=404, detail="Чат не найден")

    # Здесь можно добавить логику для переключения контекста, если это необходимо
    return JSONResponse(content={"session_id": chat.session_id, "chat_name": chat.chat_name, "collection_name": chat.collection_name})
