...
class Document(Base):
    ...
    chunks_count = Column(Integer, nullable=True) # Для картинок ставим 1

    # Новые поля для изображений
    is_image = Column(Boolean, default=False, nullable=False) # Флаг, что это изображение
    image_description = Column(Text, nullable=True) # Сгенерированное описание изображения

    # Опционально: связь с чатом
