...
# --- Функция для конденсации (реформулирования) вопроса ---
def condense_question(user_message: str, memory: ConversationBufferMemory) -> str:
    """
    Если вопрос слишком короткий или неоднозначный, то на основе истории диалога
    генерируется уточняющий запрос, который будет использован для векторного поиска.
    Здесь мы считаем, что если вопрос содержит менее 5 слов, то он нуждается в уточнении.
    """
...
async def generate_rag_response(user_message: str, search_results: list, memory: ConversationBufferMemory):
    # Если найдены данные из Qdrant, используем их как дополнительный контекст
    if search_results:
        ...
        retrieved_context = ""
    # Загружаем историю из памяти ИИ
    history = memory.load_memory_variables({}).get("history", "")
    # Формируем общий контекст: сначала история ИИ, затем релевантные данные, затем сообщение пользователя
    ...
    if search_results:
        # Выводим в консоль использованный контекст для отладки
        print("Контекст, использованный для формирования ответа:\n", combined_context)
        # Добавляем суффикс к ответу
...
async def chat_endpoint(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    # --- Сохранение сообщения пользователя в БД (для анализа) ---
    db.add(ChatMessage(session_id=session_id, role="user", message=user_message))
    db.commit()

    # --- Инициализация памяти ИИ для сессии, если отсутствует ---
    ...
    # --- Применяем конденсацию вопроса, если он кажется неоднозначным ---
    ...
    try:
        # --- Векторный поиск в Qdrant с использованием уточнённого запроса ---
...
async def clear_ai_memory(session_id: str):
    if session_id in ai_memories:
        # Пересоздаём память для сессии, тем самым очищая историю контекста
        ai_memories[session_id] = ConversationBufferMemory(memory_key="history", return_messages=True)
